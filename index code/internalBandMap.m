...
for k=1:channels
    for i=1:num_groups(k)
        ...
        [group_index,~] = find( tracking_matrix(:,1,k) == i );
        ...
        annotation_groups{i,k} = [ anno_listing(group_index) ... 
            anno_listing(group_index+1) anno_listing(group_index+2) ...
            anno_listing(group_index+3)];
    end
end
...
% build layer % maps, % that given the root group the nth group after that
% group occurs from the known data
states = length(tail_groups{1,1}(1,:));
state_percent_matrix = zeros(max(num_groups),max(num_groups),channels,states);
for k=1:channels
    for n=2:states
        for i=1:num_groups(k)
            tail_match = tail_groups{i,k}(:,n);
            for r=1:num_groups(k)
                state_percent_matrix(i,r,k,n) = sum( tail_match == r ) / num_groups(k);
            end
        end
    end
end
