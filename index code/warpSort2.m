...
% sorted_data has the distances of each associated window in increasing
% order. naturally row 0 is all zeros as they all match with themselves.
% the next row is the next closest window which is generally the distance
% to the previous or ensuing window. the window indexes are stored in
% sorted index
[sorted_data, sorted_index] = sort(data,1);

figure('name','Sorted Distances','numbertitle','off');
% this produces a mesh color plot where the colors indicate the range of
% distances present/associated with each root window
subplot(211);mesh(sorted_data);view(0,90);
xlabel('Window Index');ylabel('Number of windows');
axis([1 leg1 1 leg2]);
colormap(R.myColorMapBluePink); colorbar;
title('Raw Distance Measurements');
% now with the log of the distances taken it becomes 'more' obvious which
% root windows match with the most other windows
subplot(212);mesh(log10(sorted_data));view(0,90);
xlabel('Window Index');ylabel('Number of windows');
axis([1 leg1 1 leg2]);
colormap(R.myColorMapBluePink); colorbar;
title('Log Scaled Distance Measurements');
