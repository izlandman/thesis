...
function result = featureBuild(data,sample_frequency,start_time,stop_time,button)
...
time_set_data = normalizedWindow(plot_data,sample_frequency,start_time,stop_time);
...
% create a time series data type normalize to some value (sample rate) so
% that the vectors can be directly compared via residuals. this may be
% redundant given that signals can also be cross-correlated even with
% unique time durations/sample rates
function result = normalizedWindow(data,sample_rate,start_time,stop_time)

    window_means = round( (stop_time-start_time)/2 );
    window_start = window_means - floor(sample_rate/2) + 1;
    window_end = window_start + sample_rate - 1;

    result = data(window_start:window_end,:);
