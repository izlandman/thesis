function [Dist,D,k,w,rw,tw]=cdtw2(r,t,pflag)
%
...
while ((i+j)>2)
    ...
    elseif (j-2)<0 
        ...
        [values,number]=min([D(i,j-1) D(i-1,j) D(i-1,j-1)]);
        switch (number)
            case 1,
                if ((t(n-1)<=r(m) && r(m)<=t(n)) || (t(n)<=r(m) && r(m)<=t(n-1)))
                    x=(r(m)-t(n-1))/(t(n)-t(n-1) + 0.0000001);
            ...
            case 2,
                if ((r(m-1)<=t(n) && t(n)<=r(m)) || (r(m)<=t(n) && t(n)<=r(m-1)))
                    x=(t(n)-r(m-1))/( (r(m)-r(m-1)) + 0.0000001 );
